...
/**
 * A sequence of Environment instances.
 */
public final class EnvironmentSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public EnvironmentSeq() {
        super(Environment.class);
    }


    public EnvironmentSeq(int initialMaximum) {
        super(Environment.class, initialMaximum);
    }


    public EnvironmentSeq(Collection elements) {
        super(Environment.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, Environment.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' Environment objects (beyond the original size of this object)
